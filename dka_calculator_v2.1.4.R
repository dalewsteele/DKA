...
twoBagServer <- function(id, maint_hourly, final_deficit_maint_rate, trekk_hourly,
                         ...
                         bag1_composition, bag2_composition) {
  moduleServer(id, function(input, output, session) {
      ...
      tagList(
        ...
        layout_columns(
          ...
          value_box(
            ...
            theme = "primary",
            height = "100px",
            p(class = "small text-muted mb-0", tags$strong({
              bag1 <- bag1_composition()
              dex <- if (bag1$dextrose == 0) "No dextrose" else paste0("D", bag1$dextrose)
              saline <- paste0(bag1$saline, "% NS")
              k <- paste0(bag1$potassium, " mEq/L K+")
              paste(dex, "+", saline, "+", k)
            }))
          ),
          value_box(
            ...
            height = "100px",
            p(class = "small text-muted mb-0", tags$strong({
              bag2 <- bag2_composition()
              dex <- if (bag2$dextrose == 0) "No dextrose" else paste0("D", bag2$dextrose)
              saline <- paste0(bag2$saline, "% NS")
              k <- paste0(bag2$potassium, " mEq/L K+")
              paste(dex, "+", saline, "+", k)
            }))
          )
        )
...
ui <- page_sidebar(
  ...
  # Version badge (positioned next to title)
  tags$div(
    class = "version-badge",
  ...
  tags$style(HTML("
    ...
    [data-bs-theme='dark'] .version-display {
      color: #adb5bd;
    }
  ...
  navset_card_underline(
    ...
    nav_panel(
      ...
      card(
        card_header(
          class = "bg-secondary text-white",
          icon("flask-vial"), " Fluid Composition Defaults"
        ),
        card_body(
          tags$p(class = "text-muted", 
                 "Customize fluid compositions to match your institution's standards. These settings will be used in the Two-Bag Calculator."),
          ...
          layout_columns(
            ...
            # Bag 1 Settings
            card(
              card_header(class = "bg-primary text-white py-1", "Bag 1 (No Dextrose)"),
              card_body(
                numericInput("bag1_dextrose", 
                            "Dextrose %:", 
                            value = 0, 
                            min = 0, 
                            max = 20, 
                            step = 0.5),
                numericInput("bag1_saline", 
                            "NaCl %:", 
                            value = 0.9, 
                            min = 0, 
                            max = 3, 
                            step = 0.1),
                numericInput("bag1_potassium", 
                            "Potassium (mEq/L):", 
                            value = 40, 
                            min = 0, 
                            max = 80, 
                            step = 5),
                tags$div(
                  class = "alert alert-info py-1 px-2 mt-2",
                  style = "font-size: 0.85em;",
                  icon("info-circle"), " ",
                  uiOutput("bag1_preview")
                )
              )
            ),
            ...
            # Bag 2 Settings
            card(
              card_header(class = "bg-info text-white py-1", "Bag 2 (With Dextrose)"),
              card_body(
                numericInput("bag2_dextrose", 
                            "Dextrose %:", 
                            value = 10, 
                            min = 0, 
                            max = 20, 
                            step = 0.5),
                numericInput("bag2_saline", 
                            "NaCl %:", 
                            value = 0.9, 
                            min = 0, 
                            max = 3, 
                            step = 0.1),
                numericInput("bag2_potassium", 
                            "Potassium (mEq/L):", 
                            value = 40, 
                            min = 0, 
                            max = 80, 
          ...
          layout_columns(
            col_widths = c(6, 6),
            actionButton("resetFluidDefaults", 
                        "Reset to Standard Fluids", 
                        class = "btn-warning w-100",
                        icon = icon("rotate-left")),
            actionButton("saveFluidDefaults", 
                        "Save Settings", 
          ...
          tags$div(
            class = "mt-3",
            tags$h6("Common Presets:"),
            tags$p(class = "text-muted small mb-1", 
                   tags$strong("Standard:"), " Bag 1: 0% dextrose + 0.9% NS + 40 mEq/L K+ | Bag 2: D10 + 0.9% NS + 40 mEq/L K+"),
            tags$p(class = "text-muted small mb-1", 
                   tags$strong("Half-normal saline:"), " Bag 1: 0% dextrose + 0.45% NS + 40 mEq/L K+ | Bag 2: D10 + 0.45% NS + 40 mEq/L K+"),
            tags$p(class = "text-muted small", 
                   tags$strong("D5 variant:"), " Bag 1: 0% dextrose + 0.9% NS + 40 mEq/L K+ | Bag 2: D5 + 0.9% NS + 40 mEq/L K+")
          )
        )
      )
    )
...
# SERVER
  ...
  server <- function(input, output, session) {
    ...
    twoBagServer("twobag_module", 
                 ...
                 reactive({
                   list(
                     dextrose = input$bag1_dextrose,
                     saline = input$bag1_saline,
                     potassium = input$bag1_potassium
                   )
                 }),
                 reactive({
                   list(
                     dextrose = input$bag2_dextrose,
                     saline = input$bag2_saline,
                     potassium = input$bag2_potassium
                   )
                 }))
    
    # Settings tab - Bag previews
    output$bag1_preview <- renderUI({
      dex <- if (input$bag1_dextrose == 0) "No dextrose" else paste0("D", input$bag1_dextrose)
      saline <- paste0(input$bag1_saline, "% NS")
      k <- paste0(input$bag1_potassium, " mEq/L K+")
      tags$span(tags$strong(paste(dex, "+", saline, "+", k)))
    })
    
    output$bag2_preview <- renderUI({
      dex <- if (input$bag2_dextrose == 0) "No dextrose" else paste0("D", input$bag2_dextrose)
      saline <- paste0(input$bag2_saline, "% NS")
      k <- paste0(input$bag2_potassium, " mEq/L K+")
      tags$span(tags$strong(paste(dex, "+", saline, "+", k)))
    })
    
    # Reset fluid defaults button
    observeEvent(input$resetFluidDefaults, {
      updateNumericInput(session, "bag1_dextrose", value = 0)
      updateNumericInput(session, "bag1_saline", value = 0.9)
      updateNumericInput(session, "bag1_potassium", value = 40)
      updateNumericInput(session, "bag2_dextrose", value = 10)
      updateNumericInput(session, "bag2_saline", value = 0.9)
      updateNumericInput(session, "bag2_potassium", value = 40)
      showNotification("Reset to standard fluid compositions", 
                       type = "message", 
                       duration = 2)
    })
    
    # Save fluid defaults button (could save to localStorage via JavaScript)
    observeEvent(input$saveFluidDefaults, {
